...
public abstract class GodBoss {
    ...
    // Boss scaling modifiers
    protected double healthMultiplier = 1.0;
    protected double damageMultiplier = 1.0;
    protected double effectDurationMultiplier = 1.0;
    protected int effectLevelModifier = 0;
    protected double cooldownMultiplier = 1.0;
    protected double radiusMultiplier = 1.0;
    protected double minionSpawnMultiplier = 1.0;
    protected double abilityDamageMultiplier = 1.0;
    ...
    public void spawn(Location location) {
        ...
        if (plugin.getConfigManager().isBossScalingEnabled()) {
            org.bukkit.Difficulty difficulty = entity.getWorld().getDifficulty();
        ...
        // Apply scaled health and damage
        double scaledHealth = bossType.getMaxHealth() * healthMultiplier;
        double scaledDamage = bossType.getDamage() * damageMultiplier;
        
        entity.getAttribute(Attribute.MAX_HEALTH).setBaseValue(scaledHealth);
        entity.setHealth(scaledHealth);
        entity.getAttribute(Attribute.ATTACK_DAMAGE).setBaseValue(scaledDamage);
    ...
    private void applyDifficultyScaling(org.bukkit.Difficulty difficulty) {
        healthMultiplier = plugin.getConfigManager().getHealthMultiplier(difficulty);
        damageMultiplier = plugin.getConfigManager().getDamageMultiplier(difficulty);
        effectDurationMultiplier = plugin.getConfigManager().getEffectDurationMultiplier(difficulty);
        effectLevelModifier = plugin.getConfigManager().getEffectLevelModifier(difficulty);
        cooldownMultiplier = plugin.getConfigManager().getCooldownMultiplier(difficulty);
        radiusMultiplier = plugin.getConfigManager().getRadiusMultiplier(difficulty);
        minionSpawnMultiplier = plugin.getConfigManager().getMinionSpawnMultiplier(difficulty);
        abilityDamageMultiplier = plugin.getConfigManager().getAbilityDamageMultiplier(difficulty);
        
        plugin.getLogger().info("Boss scaling applied for " + difficulty + ": Health=" + healthMultiplier + 
            ", Damage=" + damageMultiplier + ", Effects=" + effectDurationMultiplier);
    }
    
    // Helper methods for scaled values
    protected double getScaledDamage(double baseDamage) {
        return baseDamage * abilityDamageMultiplier;
    }
    
    protected double getScaledRadius(double baseRadius) {
        return baseRadius * radiusMultiplier;
    }
    
    protected int getScaledMinionCount(int baseCount) {
        return Math.max(1, (int) (baseCount * minionSpawnMultiplier));
    }
    
    protected int getScaledEffectDuration(int baseDuration) {
        return (int) (baseDuration * effectDurationMultiplier);
    }
    
    protected int getScaledEffectLevel(int baseLevel) {
        return Math.max(0, baseLevel + effectLevelModifier);
    }
    ...
    protected long getAbilityCooldown() {
        return switch (phase) {
            case 1 -> (long) (5000 * cooldownMultiplier); // 5 seconds
            case 2 -> (long) (3000 * cooldownMultiplier); // 3 seconds
            case 3 -> (long) (2000 * cooldownMultiplier); // 2 seconds
            default -> (long) (5000 * cooldownMultiplier);
        };
    }

...
public class FallenWardenBoss extends GodBoss {
    ...
    private void soulDrain() {
        ...
        double scaledRadius = getScaledRadius(15.0);
        for (Player player : entity.getWorld().getPlayers()) {
            if (player.getLocation().distance(entity.getLocation()) <= scaledRadius) {
                double damage = getScaledDamage(4.0 * phase);
    ...
    private void deathPulse() {
        ...
        int maxRadius = (int) getScaledRadius(20.0);
                ...
                for (double angle = 0; angle < 360; angle += 10) {
                    ...
                    for (Player player : entity.getWorld().getPlayers()) {
                        if (player.getLocation().distance(particleLoc) <= 2) {
                            player.damage(getScaledDamage(6.0));
                            int duration = getScaledEffectDuration(100);
                            int level = getScaledEffectLevel(1);
                            player.addPotionEffect(new PotionEffect(PotionEffectType.WITHER, duration, level));
                        }
                    }
                }
            }, radius * 2L);
    ...
    private void necroticExplosion() {
        ...
        Location loc = entity.getLocation();
        double scaledRadius = getScaledRadius(25.0);
        ...
        for (Player player : entity.getWorld().getPlayers()) {
            if (player.getLocation().distance(loc) <= scaledRadius) {
                double distance = player.getLocation().distance(loc);
                double damage = Math.max(1, getScaledDamage(15.0) - distance);
                player.damage(damage);
                
                int blindDuration = getScaledEffectDuration(100);
                int slowDuration = getScaledEffectDuration(200);
                int slowLevel = getScaledEffectLevel(2);
                
                player.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, blindDuration, 0));
                player.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, slowDuration, slowLevel));
            }
        }
        
        // Spawn undead minions
        int minionCount = getScaledMinionCount(5);

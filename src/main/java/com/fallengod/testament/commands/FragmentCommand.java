...
public class FragmentCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "bounty" -> {
                if (args.length < 2) {
                    sender.sendMessage("§cUsage: /fragment bounty <list|clear>");
                    return true;
                }
                manageBounties(sender, args[1]);
            }
    ...
    private void showHelp(CommandSender sender) {
        ...
        sender.sendMessage("§e/fragment bounty <list|clear> §7- Manage bounty system");
    }
    ...
    private void manageBounties(CommandSender sender, String action) {
        switch (action.toLowerCase()) {
            case "list" -> {
                var bounties = plugin.getBountyManager().getActiveBounties();
                if (bounties.isEmpty()) {
                ...
                for (var entry : bounties.entrySet()) {
                    ...
                    var bounty = entry.getValue();
                    String targetName = target != null ? target.getName() : "Unknown";
                    
                    sender.sendMessage("§c" + targetName + " §7- §6" + (int)bounty.getAmount() + 
                        " diamonds §7(" + bounty.getReason() + ") - §f" + 
                        bounty.getTimeRemainingMinutes() + "m remaining");
                }
            }
            case "clear" -> {
                var bounties = plugin.getBountyManager().getActiveBounties();
                ...
                bounties.clear();
                sender.sendMessage("§aCleared " + count + " active bounties.");
            }
            default -> sender.sendMessage("§cUsage: /fragment bounty <list|clear>");
        }
    }
}
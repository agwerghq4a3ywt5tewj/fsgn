...
public class ConvergenceManager {
    ...
    private void announceConvergenceCompletion(Player player) {
        ...
        // Broadcast coordinates if enabled
        if (plugin.getConfigManager().isBroadcastEnabled() && 
            plugin.getConfigManager().isConvergenceCoordsBroadcastEnabled()) {
            
            Location nexusLoc = getPlayerNexusLocation(player.getUniqueId());
            if (nexusLoc != null) {
                String coordMessage = plugin.getConfigManager().getConvergenceActivationMessage()
                    .replace("{PLAYER}", player.getName())
                    .replace("{X}", String.valueOf(nexusLoc.getBlockX()))
                    .replace("{Y}", String.valueOf(nexusLoc.getBlockY()))
                    .replace("{Z}", String.valueOf(nexusLoc.getBlockZ()))
                    .replace("{WORLD}", nexusLoc.getWorld().getName());
                
                plugin.getServer().broadcastMessage(coordMessage);
            }
        }
        
        plugin.getServer().broadcastMessage("");
        
        // Place massive bounty on convergence achiever
        if (plugin.getConfigManager().isBountySystemEnabled()) {
            double bountyAmount = plugin.getConfigManager().getConvergenceBountyAmount();
            String reason = "Achieved Divine Convergence - Master of All Divinity";
            plugin.getBountyManager().placeBounty(player, bountyAmount, reason);
        }
